...
export default function AIPrompt() {
  // State management for AI task generation workflow
  const [prompt, setPrompt] = useState(""); // User's natural language input
  const [isLoading, setIsLoading] = useState(false); // Loading state for AI generation
  const [generatedTasks, setGeneratedTasks] = useState([]); // Tasks generated by AI
  const [showConfirmation, setShowConfirmation] = useState(false); // Controls confirmation UI
  const [successMessage, setSuccessMessage] = useState(""); // Success feedback message
  
  // Task context and navigation hooks
  const { refreshTasksAfterAI, incompleteTasks } = useTasks();
  const navigate = useNavigate();

  /**
   * Handle task generation from user prompt
   * Sends natural language input to AI service and displays generated tasks for confirmation
   */
  const handleGenerateTasks = async () => {
    ...
    setIsLoading(true);
    try {
      // Send prompt to AI service for task generation
   ...
   * Creates a confirmed task in the database and updates the UI accordingly
   * 
   * @param {number} taskIndex - Index of the task to confirm in the generated tasks array
   */
  ...
  /**
   * Handle task rejection with animation
   * Removes a rejected task from the confirmation list with smooth animation
   * 
   * @param {number} taskIndex - Index of the task to reject in the generated tasks array
   */
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#4A3C3C] to-[#3B2F2F]">
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        {/* Task Confirmation Section - Shows generated tasks for review */}
        {showConfirmation && (
